...
monthsBetween dA dB =
    if month dA == Nothing && month dB == Nothing then
        Just (abs <| 12 * (year dA - year dB))

    else if month dA == Nothing || month dB == Nothing then
        Nothing

    else
        Just (abs (12 * (year dA - year dB) + monthToInt (Maybe.withDefault Jan (month dA)) - monthToInt (Maybe.withDefault Jan (month dB))))
...
compare (Date d1) (Date d2) =
    let
        yearEquals =
            Basics.compare (d1.month |> Maybe.map monthToInt |> Maybe.withDefault 13)
                (d2.month |> Maybe.map monthToInt |> Maybe.withDefault 13)

        yearNotEquals =
            Basics.compare d1.year d2.year
    in
    Model.Util.chainCompare yearEquals yearNotEquals
...
view : Date -> Html msg
view (Date d) =
    text (String.fromInt d.year ++ " " ++ (d.month |> Maybe.map monthToString |> Maybe.withDefault ""))
